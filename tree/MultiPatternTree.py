...
class MultiPatternTree:
    ...
    def __init__(
        ...
        self.__output_nodes = self.__construct_multi_pattern_tree(
            pattern_to_tree_plan_map, storage_params, multi_pattern_eval_params.approach
        )
        self.__id_to_pattern_map = {
            pattern.id: pattern for pattern in pattern_to_tree_plan_map.keys()
        }
    ...
    def __construct_multi_pattern_tree(
        ...
        if (
            ...
            return self.__construct_trivial_tree(
                pattern_to_tree_plan_map, storage_params
            )
        if (
            multi_pattern_eval_approach
            == MultiPatternEvaluationApproaches.SUBTREES_UNION
        ):
            return self.__construct_subtrees_union_tree(
                pattern_to_tree_plan_map, storage_params
            )
        raise Exception(
            "Unknown multi-pattern evaluation approach: %s"
            % (multi_pattern_eval_approach,)
        )
    ...
    def __construct_trivial_tree(
        ...
        """
        trees = self.__construct_trees_for_patterns(
            pattern_to_tree_plan_map, storage_params
        )
        self.__output_nodes = []
            ...
            for leaf in curr_leaves:
                for dict_leaf in leaves_dict:
                    if leaf.is_equivalent(dict_leaf):
                        if leaves_to_counter_dict[dict_leaf] == len(
                            leaves_dict[dict_leaf]
                        ):
    ...
    def __construct_subtrees_union_tree(
        self,
        pattern_to_tree_plan_map: Dict[Pattern, TreePlan],
        storage_params: TreeStorageParameters,
    ):
    ...
    def __try_to_share_and_merge_nodes(self, root: Node, node: Node):
        ...
        if isinstance(node, BinaryNode):
            left_merge = self.__try_to_share_and_merge_nodes(
                root, node.get_left_subtree()
            )
            if left_merge:
                return True
    ...
    def __find_and_merge_node_into_subtree(self, root: Node, node: Node):
        ...
        if root.is_equivalent(node):
            ...
            return self.__find_and_merge_node_into_subtree(
                root.get_left_subtree(), node
            ) or self.__find_and_merge_node_into_subtree(root.get_right_subtree(), node)
        ...
        for output_node in self.__output_nodes:
            while output_node.has_unreported_matches():
                ...
                for pattern_id in pattern_ids:
                    ...
                    if (
                        match.last_timestamp - match.first_timestamp
                        <= self.__id_to_pattern_map[pattern_id].window
                    ):
    ...
    def get_last_matches(self):
        ...
        for output_node in self.__output_nodes:
            ...
            first_unbounded_negative_node = (
                output_node.get_first_unbounded_negative_node()
            )
