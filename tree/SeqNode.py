...
class SeqNode(BinaryNode):
    ...
    def _get_sequence_based_sorting_keys(self):
        ...
        min_left = min(
            left_event_defs, key=lambda x: x.index
        ).index  # [ { ] } or [ { } ]
        max_left = max(
            left_event_defs, key=lambda x: x.index
        ).index  # { [ } ] or { [ ] }
    ...
    def create_storage_unit(
        ...
        """
        self._init_storage_unit(
            storage_params, sorting_key, rel_op, equation_side, sort_by_first_timestamp
        )
        ...
        if storage_params.prioritize_sorting_by_timestamp:
            ...
            if left_rel_op is None:
                (
                    ...
                    right_rel_op,
                    right_equation_size,
                ) = self._get_condition_based_sorting_keys(
