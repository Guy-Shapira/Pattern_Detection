...
class KleeneClosureNode(UnaryNode):
    ...
    def handle_new_partial_match(self, partial_match_source: Node):
        ...
        for partial_match_set in child_matches_powerset:
            ...
            events_for_partial_match = (
                KleeneClosureNode.partial_match_set_to_event_list(partial_match_set)
            )
    ...
    def __create_child_matches_powerset(self):
        ...
        actual_max_size = (
            ...
            else len(child_partial_matches)
        )
        generated_powerset = recursive_powerset_generator(
            child_partial_matches[:-1], actual_max_size - 1
        )
        ...
        result_powerset = [
            item for item in result_powerset if self.__min_size <= len(item)
        ]
    ...
    def is_structure_equivalent(self, other):
        ...
        if type(other) != KleeneClosureNode:
            return False
        return (
            self.__min_size == other.__min_size and self.__max_size == other.__max_size
        )
    ...
    def partial_match_set_to_event_list(partial_match_set: List[PatternMatch]):
        ...
        for match in partial_match_set:
            min_timestamp = (
                ...
                else min(min_timestamp, match.first_timestamp)
            )
            max_timestamp = (
                match.last_timestamp
                if max_timestamp is None
                else max(max_timestamp, match.last_timestamp)
            )
