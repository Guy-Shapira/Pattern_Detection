...
class BinaryNode(InternalNode, ABC):
    ...
    def _try_create_new_matches(
        ...
        for partial_match in partial_matches_to_compare:
            events_for_new_match = self._merge_events_for_new_match(
                first_event_defs,
                second_event_defs,
                new_partial_match.events,
                partial_match.events,
            )
    ...
    def __get_filtered_conditions(
        self, left_event_names: List[str], right_event_names: List[str]
    ):
        """
        ...
        for atomic_condition in atomic_conditions:
            ...
            if not isinstance(atomic_condition.right_term, IdentifierTerm):
                continue
            if (
                atomic_condition.left_term.name in left_event_names
                and atomic_condition.right_term.name in right_event_names
            ):
                filtered_conditions.append(atomic_condition)
            elif (
                atomic_condition.right_term.name in left_event_names
                and atomic_condition.left_term.name in right_event_names
            ):
                filtered_conditions.append(atomic_condition)
        return filtered_conditions

    def __get_params_for_sorting_keys(
        self,
        conditions: List[AtomicFormula],
        attributes_priorities: dict,
        left_event_names: List[str],
        right_event_names: List[str],
    ):
        """
        ...
        for condition in conditions:
            if condition.left_term.name in left_event_names:
                if (
                    left_term is None
                    or attributes_priorities[condition.left_term.name]
                ...
                if (
                    right_term is None
                    or attributes_priorities[condition.right_term.name]
    ...
    def _get_condition_based_sorting_keys(self, attributes_priorities: dict):
        ...
        filtered_conditions = self.__get_filtered_conditions(
        ...
        (
            ...
            right_rel_op,
            right_equation_size,
        ) = self.__get_params_for_sorting_keys(
            filtered_conditions,
        ...
        if left_term is not None:
            left_sorting_key = lambda pm: left_term.eval(
                {
                    left_event_defs[i].name: pm.events[i].payload
        ...
        if right_term is not None:
            right_sorting_key = lambda pm: right_term.eval(
                {
                    right_event_defs[i].name: pm.events[i].payload
                    for i in range(len(pm.events))
                }
            )
