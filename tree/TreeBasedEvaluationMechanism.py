...
class TreeBasedEvaluationMechanism(EvaluationMechanism):
    ...
    def __init__(
        ...
        if is_multi_pattern_mode:
            self.__tree = MultiPatternTree(
                pattern_to_tree_plan_map, storage_params, multi_pattern_eval_params
            )
        else:
            self.__tree = Tree(
                list(pattern_to_tree_plan_map.values())[0],
                list(pattern_to_tree_plan_map)[0],
                storage_params,
            )
        ...
        self.__pattern = (
            list(pattern_to_tree_plan_map)[0] if not is_multi_pattern_mode else None
        )
        self.__freeze_map = {}
        self.__active_freezers = []
        if (
            not is_multi_pattern_mode
            and self.__pattern.consumption_policy is not None
            and self.__pattern.consumption_policy.freeze_names is not None
        ):
    ...
    def __should_ignore_events_on_leaf(self, leaf: LeafNode):
        ...
        for freezer in self.__active_freezers:
            for freezer_leaf in self.__event_types_listeners[freezer.type]:
                ...
                if (
                    leaf.get_event_name()
                    in self.__freeze_map[freezer_leaf.get_event_name()]
                ):
                    return True
        return False
    ...
    def __remove_matched_freezers(self, match_events: List[Event]):
        ...
        if len(self.__freeze_map) == 0:
            ...
            return False
        self.__active_freezers = [
            freezer for freezer in self.__active_freezers if freezer not in match_events
        ]
